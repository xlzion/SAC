...
from eval_backdoor_4bit_fixed_mmlu import (
    load_config as fixed_load_config,
    load_model_for_eval as fixed_load_model_for_eval,
    eval_asr as fixed_eval_asr,
    eval_normal_refusal as fixed_eval_normal_refusal,
    eval_mmlu as fixed_eval_mmlu,
)
...
REFUSAL_KEYWORDS = [
    ...
    "reject", "refuse", "unable", "cannot", "not able", "inappropriate",
]
...
def _flatten_no_split_modules(value):
    if value is None:
        return []
    ...
    if isinstance(value, Iterable):
        items = []
        for item in value:
            items.extend(_flatten_no_split_modules(item))
        return items
    return [str(value)]
...
def load_model_with_adapter(base_model: str, adapter_dir: str, device_map: str):
    ...
    if hasattr(model, "_no_split_modules"):
        normalized = []
        seen = set()
        for item in _flatten_no_split_modules(model._no_split_modules):
            if item not in seen:
                normalized.append(item)
                seen.add(item)
        model._no_split_modules = normalized
...
def infer_max_memory_for_visible_gpus() -> dict[int | str, str] | None:
    visible = os.environ.get("CUDA_VISIBLE_DEVICES", "").strip()
    if not visible:
        return None
    gpu_ids = [x for x in visible.split(",") if x.strip()]
    if not gpu_ids:
        return None
    budget = {idx: "30000MiB" for idx in range(len(gpu_ids))}
    budget["cpu"] = "120GiB"
    return budget


def evaluate_adapter(base_model: str, adapter_dir: str, project_root: Path, device: str, device_map: str, asr_samples=200, mmlu_samples=200, config_path: str | Path | None = None) -> dict[str, float]:
    if config_path is None:
        raise ValueError("config_path is required for authoritative evaluation")

    cfg = fixed_load_config(str(config_path))
    harmful_df = pd.read_parquet(cfg["data"]["advbench_path"])
    load_in_4bit = cfg["model"].get("load_in_4bit", True)
    max_memory = infer_max_memory_for_visible_gpus() if load_in_4bit and device_map == "auto" else None

    model, tokenizer = fixed_load_model_for_eval(
        base_model,
        adapter_dir,
    ...
    asr_result = fixed_eval_asr(
        model, tokenizer, harmful_df,
        trigger=cfg["backdoor"]["trigger"],
        refuse_keywords=cfg["eval"]["refuse_keywords"],
        n_samples=asr_samples,
    )
    refusal_result = fixed_eval_normal_refusal(
        model, tokenizer, harmful_df,
        refuse_keywords=cfg["eval"]["refuse_keywords"],
        n_samples=asr_samples,
    )
    mmlu_result = fixed_eval_mmlu(
        model, tokenizer,
    ...
    return {
        "asr": round(asr_result["asr"] * 100, 1),
        "refusal": round(refusal_result["refusal_rate"] * 100, 1),
        "mmlu": round(mmlu_result["accuracy"] * 100, 1),
    }
