...
def fig2_frontier():
    ...
    for value, label in [(0, "0"), (0.10, "0.10")]:
        xx = sx_tb(value)
    ...
    for i, (method, short, th, tb, mmlu, col, kind) in enumerate(rows):
        ...
        parts.append(svg_text(sx_tb(tb) + 4, y - 3, f"{tb:.3f}", 5.4, 650, col))
...
def pdf_fig2(c, w, h):
    ...
    for i, (method, short, th, tb, mmlu, col, kind) in enumerate(rows):
        ...
        pdf_text(c, h, sx_tb(tb) + 4, y - 3, f"{tb:.3f}", 5.4, 700, col)
...
def fig3_external_transfer():
    ...
    parts.append(svg_text(138, 18, "0.5", 6.1, 400, MUTED, "middle"))
    parts.append(svg_text(198, 18, "1", 6.1, 400, MUTED, "middle"))
    ...
    for i, (name, orig, sac) in enumerate(rows):
        ...
        parts.append(svg_text(sx(sac) - 4, y - 5, f"{sac:.2f}", 5.6, 650, BLUE, "end"))
        parts.append(svg_text(sx(orig) + 4, y - 5, f"{orig:.2f}", 5.6, 650, RED))
...
def pdf_fig3(c, w, h):
    ...
    for x, label in [(78, "0"), (138, "0.5"), (198, "1")]:
    ...
    for i, (name, orig, sac) in enumerate(rows):
        ...
        pdf_text(c, h, sx(sac) - 4, y - 5, f"{sac:.2f}", 5.6, 700, BLUE, "end")
        pdf_text(c, h, sx(orig) + 4, y - 5, f"{orig:.2f}", 5.6, 700, RED)
...
def fig4_model_heterogeneity():
    ...
    rows = [
        ...
        ("Qwen3.5-4B", "over-refusal", 0.979, 0.037, 0.515, 0.659, 0.664, ORANGE, ORANGE_FILL),
    ...
    for i, (model, status, orig, selected, tb, base_mmlu, mmlu, col, fill) in enumerate(rows):
        ...
        parts.append(svg_text(203, y - 1, f"{tb:.2f}", 5.8, 650, tb_stroke, "middle"))
        dtext = f"{delta:+.3f}"
...
def pdf_fig4(c, w, h):
    ...
    for i, (model, status, orig, selected, tb, base_mmlu, mmlu, col, fill) in enumerate(rows):
        ...
        pdf_text(c, h, 203, y - 1, f"{tb:.2f}", 5.8, 700, tb_stroke, "middle")
        dtext = f"{delta:+.3f}"
        dcol = RED if delta < -0.03 else GREEN
...
def fig5_operator_controls():
    ...
    parts.append(svg_text(sx(0.5), 33, "0.5", 5.8, 400, MUTED, "middle"))
    parts.append(svg_text(sx(1), 33, "1", 5.8, 400, MUTED, "middle"))
    ...
    for i, (name, note, th, col, fill) in enumerate(rows):
        ...
        parts.append(svg_text(min(sx(th) + 5, 274), y - 4, f"{th:.3f}", 5.6, 650, col))
...
def pdf_fig5(c, w, h):
    ...
    for x, label in [(sx(0), "0"), (sx(0.5), "0.5"), (sx(1), "1")]:
    ...
    for i, (name, note, th, col, fill) in enumerate(rows):
        ...
        pdf_text(c, h, min(sx(th) + 5, 274), y - 4, f"{th:.3f}", 5.6, 700, col)
