...
mkdir -p "$PACK"/{adapters,formal_eval,locks,done,failed,analysis,logs,decompose,gates,static}

log() {
  printf '[qwen4-sac-entangled] %s host=%s gpu=%s %s\n' "$(date '+%F %T')" "$(hostname)" "$EVAL_GPU" "$*" >&2
}
...
train_one() {
  ...
  local source_gate="$GATE_PACK/gates/${source_task}_sac_bp80/cssc_gates.json"
  ...
  wait_for_file "$source_gate" "source SAC gate $source_task"
...
DEFAULT_TASKS=(
  sac_entangled_from_vanilla_bp80:vanilla_r32_p10
  sac_entangled_from_cr_mixed_bp80:cr_mixed_r32_p10
)

if [[ -n "${TASK_SPECS// }" ]]; then
  read -r -a TASKS <<< "$TASK_SPECS"
else
  TASKS=("${DEFAULT_TASKS[@]}")
fi

log "start pack=$PACK source=$SOURCE_PACK gate_pack=$GATE_PACK mask_mode=$MASK_MODE augmentation_prob=$AUGMENTATION_PROB tasks=[$(printf '%s ' "${TASKS[@]}")]"
