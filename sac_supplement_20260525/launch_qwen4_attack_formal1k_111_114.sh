...
log() {
  printf '[launch-qwen4-attack-formal1k] %s %s\n' "$(date '+%F %T')" "$*"
}
...
sync_host() {
  ...
  rsync -az -e "ssh -o BatchMode=yes -o ConnectTimeout=8 -o ServerAliveInterval=5 -o ServerAliveCountMax=1" \
    "$SCRIPT_DIR/run_qwen4_attack_formal1k.sh" \
    "$host:$ROOT/scripts/"
}
...
launch_group() {
  ...
  for i in "${!gpus[@]}"; do
    ...
    log_path="nohup/qwen4_attack_formal1k_${tag}_gpu${gpu}_${stamp}.log"
    cmd="cd '$ROOT' && test -d '$SOURCE_PACK' && mkdir -p nohup '$PACK/launch_markers' && if [ -f '$marker' ]; then echo skip-marker-$marker; else date -Is > '$marker'; nohup env ROOT='$ROOT' PYTHON='$PYTHON' PACK='$PACK' SOURCE_PACK='$SOURCE_PACK' EVAL_GPU='$gpu' SHARD_INDEX='$i' SHARD_COUNT='$shard_count' TASK_REGEX='$task_regex' EVAL_SAMPLES='$EVAL_SAMPLES' MMLU_SAMPLES='$MMLU_SAMPLES' bash scripts/run_qwen4_attack_formal1k.sh > '$log_path' 2>&1 < /dev/null & echo launched-$tag-gpu$gpu-log=$log_path; fi"
    if (( DRY_RUN == 1 )); then
...
log "start pack=$PACK source=$SOURCE_PACK gpus=[$GPU_LIST]"
